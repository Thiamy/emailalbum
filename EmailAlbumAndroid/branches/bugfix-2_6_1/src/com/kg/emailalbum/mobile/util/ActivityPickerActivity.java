...
/**
 * Presents a list of activities to choose from. This list only contains
 * activities that have ACTION_MAIN, since other types may require data as
 * input.
 */
public class ActivityPickerActivity extends ListActivity {
     ...
     * ArrayAdapter's built in filtering logic, which depends on toString().
     */
    ...
    private final class LoadingTask extends AsyncTask<Intent, Object, ActivityAdapter> {
        @Override
        ...
        @Override
        public ActivityAdapter doInBackground(Intent... intents) {
            // Load the activities
            Intent queryIntent = intents[0];
    ...
    protected void onCreate(Bundle savedState) {
        ...
        Intent queryIntent = (Intent)getIntent().getParcelableExtra(EXTRA_QUERY_INTENT);

        // Start loading the data
        new LoadingTask().execute(queryIntent);
    }
...
//        }
        ...
        // getResources().getResourceEntryName(info.getIconResource());
