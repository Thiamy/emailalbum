			...
			try {
				...
				while (mContinueCreation && iPictures.hasNext()) {
					...
					if (!files.contains(thumbName)) {
						InputStream imageIS = mArchive.getInputStream(entry);
						...
						dstW = 80;
						dstH = (int) (dstW / ratio);
						Bitmap thumb = Bitmap.createScaledBitmap(source
								.getBitmap(), dstW, dstH, true);

						OutputStream thumbOS = mContext.openFileOutput(
								thumbName, Context.MODE_PRIVATE);
